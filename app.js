...
const NODE_TOP_PADDING =
  0;

const NODE_BOTTOM_PADDING =
  156;
...
let draggingCharacterId =
  null;

let dragStartPointerX =
  0;

let dragStartPointerY =
  0;
...
const relationshipLineLayer =
  document.getElementById(
    "relationshipLineLayer"
  );

const relationshipMarkerLayer =
  document.getElementById(
    "relationshipMarkerLayer"
  );
...
const enterArrangeModeButton =
  document.getElementById(
    "enterArrangeModeButton"
  );

const resetWorldLayoutButton =
  document.getElementById(
    "resetWorldLayoutButton"
  );
...
const resetAllDistributionsEqualButton =
  document.getElementById(
    "resetAllDistributionsEqualButton"
  );
...
function applyAdornmentClasses(
  decorationElement,
  wrapElement,
  adornment,
  glow
...
function replaceDistributionWithEqual(
  trait
) {
...
resetAllDistributionsEqualButton.addEventListener(
  ...
  function() {
    ...
    showToast(
      "All colors reset to equal probability"
    );
...
function updateGeneticModelTotals() {
  ...
  overallWeightTotal.classList.toggle(
    "valid",
    approximately100(overall)
  );
  ...
  parentWeightTotal.classList.toggle(
    "valid",
    approximately100(parent)
  );
...
function saveLayerSettingsFromUI() {
...
function confirmResetAllPositions() {

  openConfirmation(
    "Reset all tree positions?",
    "Every manually arranged character will return to the automatic family-tree layout.",
    function() {
...
function renderCharacterNode(
  ...
  if (
    character.adornment &&
    character.adornment !== "none"
  ) {

    decoration.classList.add(
      `adornment-${character.adornment}`
    );
  ...
  node.addEventListener(
    ...
    function(event) {
      ...
      if (
        ...
        showToast(
          "This character's position is locked"
        );
      ...
      person.spouseIds
        .forEach(
          spouseId => {
            ...
            addSvgPath(
              route,
              `tree-line spouse-line-outer ${focus}`
            );

            addSvgPath(
              route,
              `tree-line spouse-line-inner ${focus}`
            );
...
function drawSimpleRelationshipLines(
  positions,
  field,
  className
) {
...
function drawParentChildGroups(
  positions
) {
...
function drawChildGroup(
  ...
  addSvgPath(
    ...
    `tree-line parent-child-line ${focus}${extra}`
  );
...
function routeHorizontalRelationship(
  first,
  second
) {
...
function routeBetweenNodes(
  ...
  if (
    ...
    return routeHorizontalRelationship(
      first,
      second
    );
...
function refreshAdornmentPreview() {
  ...
  adornmentPreviewDecoration.className =
    "node-adornment";

  adornmentPreview.classList.remove(
    "divine-glow"
  );
  ...
  if (
    ...
    adornmentPreviewDecoration.classList.add(
      `adornment-${adornment}`
    );
  ...
  if (
    ...
    adornmentPreview.classList.add(
      "divine-glow"
    );
...
applyWheelResultButton.addEventListener(
  ...
  function() {
    ...
    const select =
      activeWheelTrait === "hair"
        ? editHairPreset
        : activeWheelTrait === "eyes"
          ? editEyePreset
          : editSkinPreset;
      ...
      function() {
        ...
        characters.forEach(
          person => {
            ...
            person.siblingIds =
              person.siblingIds.filter(
                value =>
                  value !== id
              );

            person.spouseIds =
              person.spouseIds.filter(
                value =>
                  value !== id
              );

            person.loverIds =
              person.loverIds.filter(
                value =>
                  value !== id
              );
